...
import ServerGameStartUpMgr

g_baseAppEntity = None
g_serverGameStartUpMgr = None
g_isBootstrap = False
...
def onBaseAppReady(isBootstrap):
	...
	if isBootstrap:
		g_isBootstrap = True
		#注册第一个BaseAppEntity
		KBEngine.baseAppData[ Const.GLOBAL_BASEDATA_FIRST_START_BASEAPP ] = g_baseAppEntity
		g_baseAppEntity.doFirstStartBaseAppEntityRegister( g_baseAppEntity )
		#在一个个BaseAPP上创建服务器游戏启动管理器并初始化
		g_serverGameStartUpMgr = KBEngine.createEntityLocally( "ServerGameStartUpMgr", {} )
		g_serverGameStartUpMgr.init()
	
	if KBEngine.baseAppData.has_key(Const.GLOBAL_BASEDATA_FIRST_START_BASEAPP):
		KBEngine.baseAppData[ Const.GLOBAL_BASEDATA_FIRST_START_BASEAPP ].doFirstStartBaseAppEntityRegister( g_baseAppEntity )
...
def onBaseAppData(key, value):
	...
	if key == Const.GLOBAL_BASEDATA_FIRST_START_BASEAPP and g_baseAppEntity:
		KBEngine.baseAppData[ Const.GLOBAL_BASEDATA_FIRST_START_BASEAPP ].doFirstStartBaseAppEntityRegister( g_baseAppEntity )
