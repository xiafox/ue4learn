...
#include "ParticleModuleSpotLightEX.generated.h"

/*
* 文件名称：ParticleModuleSpotLightEX.h
* 功能说明：聚光灯粒子，模拟UParticleModulePointLight写的，PointLight的类型改为了SpotLight
* 文件作者：zhengxuemei
* 目前维护：
* 创建时间：2019-01-02
*/

struct FSpotLightParticlePayload : public FLightParticlePayload
{
	float InnerConeAngle;
	float OuterConeAngle;
};
struct FParticleEmitterInstance;
UCLASS(editinlinenew, hidecategories = Object, MinimalAPI, meta = (DisplayName = "SpotLight"))
class UParticleModuleSpotLightEX : public UParticleModuleBaseEX
{
	...
	UPROPERTY(EditAnywhere, Category = Light)
		struct FRawDistributionFloat InnerConeAngle;
	UPROPERTY(EditAnywhere, Category = Light)
		struct FRawDistributionFloat OuterConeAngle;
	/**
...
private:
	...
	uint64 SpawnHQLight(const FSpotLightParticlePayload& Payload, const FBaseParticle& Particle, FParticleEmitterInstance* Owner);
	void UpdateHQLight(USpotLightComponent* PointLightComponent, const FSpotLightParticlePayload& Payload, const FBaseParticle& Particle, int32 ScreenAlignment, FVector ComponentScale, bool bLocalSpace, FSceneInterface* OwnerScene, bool bDoRTUpdate);
