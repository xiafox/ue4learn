...
namespace KBEngine
{
	...
	void KBEngineApp::Login(const FString& username, const FString& password, const TArray<uint8>& datas)
	{
		...
		loginApp_->Connect(LoginappHost(), LoginappPort(), std::bind(&KBEngineApp::OnConnectToLoginappCB, this, std::placeholders::_1, std::placeholders::_2, TEXT("Login")));
	}

	void KBEngineApp::OnConnectToLoginappCB(int32 code, const TArray<uint8>& serverDatas, FString key)
	{
		if (code != (int32)ERROR_TYPE::SUCCESS)
		{
			if (KBEPersonality::Instance())
				KBEPersonality::Instance()->OnLoginFailed(code, KBEErrors::ErrorName(code), KBEErrors::ErrorDesc(code), serverDatas);
			return;
		}

		if (key == TEXT("Login"))
			loginApp_->Login(username_, password_, clientdatas_, args_->clientType, std::bind(&KBEngineApp::OnLoginToLoginappCB, this, std::placeholders::_1, std::placeholders::_2));
		else if (key == TEXT("CreateAccount"))
			loginApp_->CreateAccount(username_, password_, clientdatas_, std::bind(&KBEngineApp::OnCreateAccountCB, this, std::placeholders::_1, std::placeholders::_2));
		else if (key == TEXT("ResetPassword"))
			loginApp_->ResetPassword(username_, std::bind(&KBEngineApp::OnResetPasswordCB, this, std::placeholders::_1, std::placeholders::_2));
		else
			KBE_ERROR(TEXT("KBEngineApp::OnConnectToLoginappCB: unknown key '%s'"), *key);
	}

	void KBEngineApp::OnLoginToLoginappCB(int32 code, const TArray<uint8>& serverDatas)
	{
		if (code != (int32)ERROR_TYPE::SUCCESS)
		{
			if (KBEPersonality::Instance())
	...
	void KBEngineApp::CreateAccount(const FString& username, const FString& password, const TArray<uint8>& datas)
	{
		...
		loginApp_ = new LoginApp(this);
		loginApp_->Connect(LoginappHost(), LoginappPort(), std::bind(&KBEngineApp::OnConnectToLoginappCB, this, std::placeholders::_1, std::placeholders::_2, TEXT("CreateAccount")));
	}

	void KBEngineApp::OnCreateAccountCB(int32 code, const TArray<uint8>& serverDatas)
	{
		if (code != (int32)ERROR_TYPE::SUCCESS)
		{
			if (KBEPersonality::Instance())
	...
	void KBEngineApp::ResetPassword(const FString& username)
	{
		...
		loginApp_ = new LoginApp(this);
		loginApp_->Connect(LoginappHost(), LoginappPort(), std::bind(&KBEngineApp::OnConnectToLoginappCB, this, std::placeholders::_1, std::placeholders::_2, TEXT("ResetPassword")));
	}

	void KBEngineApp::OnResetPasswordCB(int32 code, const TArray<uint8>& serverDatas)
	{
		if (code != (int32)ERROR_TYPE::SUCCESS)
		{
			if (KBEPersonality::Instance())
