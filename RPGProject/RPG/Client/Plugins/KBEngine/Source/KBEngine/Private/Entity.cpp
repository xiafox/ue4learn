...
DECLARE_CYCLE_STAT(TEXT("KBEngine_Entity_InitProperties"), STAT_KBEngine_Entity_InitProperties, STATGROUP_KBEngine_Entity);
DECLARE_CYCLE_STAT(TEXT("KBEngine_Entity_BaseCall"), STAT_KBEngine_Entity_BaseCall, STATGROUP_KBEngine_Entity);
DECLARE_CYCLE_STAT(TEXT("KBEngine_Entity_CellCall"), STAT_KBEngine_Entity_CellCall, STATGROUP_KBEngine_Entity);
...
namespace KBEngine
{
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_KBEngine_Entity_InitProperties);
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_KBEngine_Entity_BaseCall);
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_KBEngine_Entity_CellCall);
		if (!KBEngineApp::app->pBaseApp())
		{
	...
	bool Entity::IsMultCastProperty(const FString& propertyname)
	{
		auto** p = defpropertys_.Find(propertyname);
		Property *obj = p ? *p : nullptr;
		if (obj != nullptr)
		{
			return obj->IsAllClients() || obj->IsOtherOnly();
		}
		else
		{
			return false;
	...
	void Entity::UpdateVolatileDataToServer(const FVector& pos, const FVector& dir)
	{
		...
		if (FVector::Dist(position_, pos) > 1.0f)
		{
			position_ = pos;
			posChanged = true;
		...
		if (FVector::Dist(direction_, dir) > 1.0f)
		{
			direction_ = dir;
			dirChanged = true;
